...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/sk.html')) {
    console.log("✅ sk.html found in lib folder");
  } else {
    console.log("⚠️ sk.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("sk-MD~")) {
    console.log("❌ Invalid sk-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ sk-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'shahid'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/shahid.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://i.ibb.co/GvNY7Kb1/4f0ad87c3da7.jpg'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 sk-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ sk-MD Bot connected to WhatsApp!");
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://i.ibb.co/GvNY7Kb1/4f0ad87c3da7.jpg');
